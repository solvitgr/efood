...
class KitchenController extends Controller
{
    ...
    {
        $chef_branch= ChefBranch::where('user_id', auth()->user()->id)->first();
        $branch_id = Branch::where('id', $chef_branch->branch_id)->first();
        //dd($branch_id->id);
    ...
    public function search(Request $request)
    {
        ...
        $orders = Order::where('branch_id', $branch_id->id)
    ...
    public function filter_by_status(Request $request)
    {
        ...
        if ($order_status == 'cooking'){
            $orders = Order::where(['order_status' => $order_status, 'branch_id' => $branch_id->id])
    ...
    public function get_order_details(Request $request)
    {
        ...
        $order = Order::with('table')->where(['id' => $request->order_id])->first();
        if (isset($order)){
    ...
    public function get_profile()
    {
        ...
        $chef_branch= ChefBranch::where('user_id', auth()->user()->id)->first();
        //dd($chef_branch);
        $branch = Branch::where('id', $chef_branch->branch_id)->first();
        //dd($branch);
        return response()->json([
            'profile' => $kitchen,
            'branch' => $branch
